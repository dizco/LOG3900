//
//  TransformStrokeActionStrategy.swift
//  PolyPaintPro
//
...
final class TransformStrokeActionStrategy: StrokeActionStrategy {
    func applyReceived(scene: StrokeEditorScene, incomingAction: IncomingActionMessage) {
        if !incomingAction.delta.add.isEmpty {
            for transformedStroke in incomingAction.delta.add {
                // 1. we delete the stroke
                scene.eraseByStrokeWith(strokeUuid: transformedStroke.strokeUuid)

                // 2. we replace it
                self.drawReceived(scene: scene, stroke: transformedStroke)
            }
        }
    }
